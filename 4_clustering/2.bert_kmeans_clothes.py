...
df = pd.read_excel("./data/衣服数据_deal.xlsx")
...
topics = []
for keywords in keywords_per_cluster[0]:
    topic = []
    for word in jieba.cut(keywords):
        if word in dictionary.token2id:
            topic.append(dictionary.token2id[word])
    topics.append(topic)
...
plt.title('BERT KMeans Clothes')
plt.legend(*scatter.legend_elements())
plt.savefig("./pic/BERT_KMeans_clothes.png")
